...
**Error vs Defecto**
Ambos son una alerta de mal funcionamiento, la diferencia es en que etapa se detecta.
Misma etapa donde se genera: Error, etapa siguiente donde se genera: Defecto.

**Testing**
Etapa donde se detectan defectos, es una actividad destructiva.

**Requerimiento**
deben ser objetivos y verificables, caracteristicas principales

**Metodologia verificar por par**
se caracteriza por encontrar errores con la verificacion de un par (otro desarrollador)

**Falla**
lo que le ocurre al software por culpa nuestra, le introducimos errores, defectos.

**Testing sistematico (metodologico) vs testing ad hoc**
ad hoc genera errores que no se pueden volver a reproducir
sistematico describe una secuencia para reproducir el error

**ciclo de prueba**
se corren la cantidad de casos de prueba que desarrolle para el testing y cuando se ejecutan todos se completa el ciclo.

**Validacion -> usuario (si es el sistema correcto)**

**Verificacion -> (si funciona correctamente)**

**estrategia de regresion (con regresion vs sin regresion)**
por cada error corregido, se introducen 3 estadisticamente
regresion? si, volver a probar los casos de prueba
se puede hacer selectivo? si, pero hay posibilidades que no detectar nuevos errores.

**Niveles de prueba (desde lo particular a lo general)**

- pruebas unitarias (si no se plica tdd, se hacen en implementacion)
- prueba de interfaces o integracion (tambien en implementacion)
- pruebas de sistema o version (se hacen en las PRUEBAS)
- pruebas de aceptacion de usuario (se hacen en el despliegue)

**Tipos de pruebas** (son una banda se listan unos pocos)
relacionado con los requerimientos que queremos probar si se cumplen

- operacion normal (RF)
- Negativas (RF que fallan)
- procesos de negocio (RF)
- documentacion (manual de usuario, manual de configuracion [posible instalacion])
- prueba de interfaces de usuario [RNF]
- prueba de escala completa [RNF]
- pruebas de estres (no se espera que se pase pero que se vea como queda el sistema)[RNF]
- pruebas de usabilidad [RNF]
- pruebas de accesibilidad [RNF]
  Estos se llevan a cabo en Niveles de prueba de sistemas y pruebas de usuario

**Metodos**
a. caja negra
b. caja blanca
lo que se busca es cobertura, distintos metodos para diseñar la menor cantidad de casos de prueba
